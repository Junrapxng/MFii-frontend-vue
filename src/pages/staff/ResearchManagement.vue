                  ...
                  <v-container class="flex">
                    <v-card v-for="(img, index) in currentResearch.filePath" :key="index" class="mx-2"
                      ...
                      <template v-else>
                        ...
                        <v-img :src="`http://server:7770/${img}`" v-model="currentResearch.filePath" height="250px"
...
export default {
  ...
  methods: {
    ...
    async toggleVisibility(item) {
      ...
      try {
        await axios.patch(`http://server:7770/staff/updateResearchData/${item._id}`, { status: item.status }, {
          headers: {
    ...
    async saveResearch() {
      try {
        ...
        if (this.isEdit) {
          try {
            await axios.patch(`http://server:7770/staff/updateResearchData/${this.currentResearch._id}`, {
            ...
            if (this.markedForDeletion.length > 0) {
              const deleteRequests = this.markedForDeletion.map(index => {
                ...
                return axios.patch(`http://server:7770/staff/deleteFileResearch/research/${this.currentResearch._id}`, {
                  filePath: img
                }, {
                  headers: {
                    Authorization: localStorage.getItem('token'),
            ...
            // Add file edit
            await axios.patch(`http://server:7770/staff/addFileResearch/${this.currentResearch._id}`, formData, {
              headers: {
                Authorization: localStorage.getItem('token'),
          ...
          await axios.post('http://server:7770/staff/addResearch', formData, {
            headers: {
              Authorization: localStorage.getItem('token'),
              'Content-Type': 'multipart/form-data'
            },
    ...
    async deleteResearch() {
      try {
        await axios.delete(`http://server:7770/staff/deleteResearch/research/${this.currentResearch._id}`, {
          headers: {
            Authorization: localStorage.getItem('token'),
          },
        });
    ...
    async fetchResearches() {
      try {
        const response = await axios.get('http://server:7770/getsResearch/all/all/all/all');
