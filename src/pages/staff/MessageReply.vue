...
export default {
  ...
  methods: {
    async openReplyDialog(id) {
      try {
        const response = await axios.get('http://server:7770/mesDetail/' + id, {
          headers: {
    ...
    async replyMessage(message, id) {
      try {
        await axios.patch('http://server:7770/mesReplyUpdate/' + id, {
          messages: message,
    ...
    async fetchMessages() {
      try {
        const response = await axios.get('http://server:7770/mesGetData', {
          headers: {
            Authorization: localStorage.getItem("token"),
          },
        });
