              ...
              <v-row>
                <v-col v-for="(img, index) in imgs" :key="index" cols="12" md="4">
                  <v-card class="hover:shadow-lg transition-shadow rounded-lg my-1 mx-1" style="max-width: 400px">
                    <v-img v-if="img.filePath.length > 0" :src="`http://192.168.10.184:7770/${img.filePath}`" height="150px" cover />
            ...
            <v-carousel-item v-for="(item, index) in imgs" :key="index">
              <v-sheet class="d-flex align-center justify-center" height="100%" elevation="10">
                ...
                <v-img v-else-if="item.filePath.length > 0"
                      :src="`http://192.168.10.184:7770/${item.filePath}`"
  ...
  async addNews() {
    try {
      ...
      if (formData.has('images[0]') || formData.has('linkVideo') || formData.has('linkImage')) {
        const response = await axios.post('http://192.168.10.184:7770/staff/addNews', formData, {
          headers: {
    ...
    async fetchImg() {
      ...
      try {
        const res = await axios.get('http://192.168.10.184:7770/getsNews');
    ...
    async deleteImage() {
      try {
        const response = await axios.delete(`http://192.168.10.184:7770/staff/deleteNews/news/${this.deleteImgId}`, {
          headers: {
            Authorization: localStorage.getItem("token"),
          }
        });
